...
def get_problem_f_re(name, *args, **kwargs):
    name = name.lower()
...
class simpleSyn(): 
    ...
    def get_ps(self, n_point: int = 100):
        x1 = torch.linspace(0, 1, n_point)
        x3 = torch.sin(10 * (x1 - 0.5))
        x2 = torch.sin(10 * (x1 - 0.5)) + .5*x3
        x2 = torch.where(x2<-1, -1., x2) 
        x2 = torch.where(x2>1, 1., x2)
        x = torch.vstack([x1, x2, x3.repeat(self.n_dim-2, 1)]).T
        return (x - self.lbound) / (self.ubound - self.lbound)
        
    def evaluate(self, x):

        if x.device.type == 'cuda' and self.lbound.device.type == 'cpu':        
            self.lbound = self.lbound.to(x.device.type)
            self.ubound = self.ubound.to(x.device.type)

        n = x.shape[1]
        x = x * (self.ubound - self.lbound) + self.lbound

        sum1 = torch.pow(x[:,1]-torch.sin(10*(x[:,0]-0.5))-.5*x[:,2], 2)
        sum2 = 0.0 
        for i in range(2, n): 
            sum2 += torch.pow(x[:,i]-torch.sin(10*(x[:,0]-0.5)), 2) 
        g = (sum1+sum2) / (n-1) 

        f1 = x[:,0]  
        f2 = (1 + g) * (1 - torch.sqrt(f1 / (1+g)))
...
class RE21():
    def __init__(self, n_dim = 4):
        ...
        self.ideal_point = [1237.8414230005742, 0.002761423749158419]
...
class RE33():
    def __init__(self, n_dim = 4):
        ...
        self.nadir_point = [-0.721525, 1.13907203907, 0.0]
...
class RE36():
    def __init__(self, n_dim = 4):
        ...
        self.nadir_point = [7.89473684213e-05, 12.0, 0.0]
...
class RE37():
    def __init__(self, n_dim = 4):
        ...
        self.nadir_point = [0.00889341391106, 0.00488, -0.431499999825]
